...
class Railtimes(object):
	...
	@cherrypy.expose
	def train(self, train_uid, date = datetime.now().strftime("%Y%m%d"), format = None):

		# Only bring back services running on current DoW.
		runs_day = datetime.strptime(date, "%Y%m%d").strftime('%A').lower()
		runs = {runs_day: True}
		...
		train = session.query(
							...
							Trip.service
						 	...
						 	Calendar.start_date <= date, Calendar.end_date >= date
						).filter_by(
						 	**runs
						).order_by(
