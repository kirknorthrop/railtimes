...
class HelloWorld(object):
	...
	def train(self, train_uid, date = datetime.now().strftime("%Y-%m-%d"), all_points = False, all_times = False, format = None):
		...
		if train.filter_by(stp_indicator = 'N').count():
			train = train.filter_by(stp_indicator = 'N').all()
		elif train.filter_by(stp_indicator = 'O').count():
			train = train.filter_by(stp_indicator = 'O').all()
	...
	def station(self, stanox, start_time = None, end_time = None, date = None, format = None):
		...
		if format is None:
			...
			#MIME Type		
			return json.dumps(trains_to_render, default=date_handler)
